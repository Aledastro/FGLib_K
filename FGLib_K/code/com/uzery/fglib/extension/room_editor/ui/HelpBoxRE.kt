...
class HelpBoxRE(private val data: DataRE): InfoBox() {
    ...
    override fun getL(): ArrayList<String> {
        ...
        res.add("[F1] = show/hide help window")
        res.add("")
        ...
        res.add("[CONTROL] + [A] = select all in edit room")
        res.add("[CONTROL] + [SHIFT] + [A] = select all in world")
        res.add("[CONTROL] + [D] = unselect all")
        res.add("[DELETE] = remove select objs")
        res.add("")
        res.add("= -- edit:")
        res.add("[ARROWS] / [CONTROL] + [LMB] = move select objs")
        res.add("[CONTROL] + [RMB] = copy select objs")
        res.add("")
        res.add("= -- global:")
        ...
        res.replaceAll { input ->
            if(input.startsWith("=") || input.isEmpty()) input
            else input.replace(" = ", " =       ")
        }
